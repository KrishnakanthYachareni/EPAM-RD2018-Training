...
import com.epam.rd.weektwo.autoparkingv2.sevices.Validator;
 ...
 */
/**
 * The Unpark class has unpark method it can ask for vehicle registration number then it will
 * validates by validationVehicleNo(String). After that search for vehicle in
 * parking Has set whether the given vehicle present or not if its present vehicle
 * will remove from list. otherwise will display some message.
 * 
 * @throws InvalidRegistrationNumberException
 *             InvalidRegistrationNumberException raised if vehicle no not in format
 */
public class Unpark implements Vehicle {

	/* (non-Javadoc)
	 * @see com.epam.rd.weektwo.autoparkingv1.Vehicle#doTask(com.epam.rd.weektwo.autoparkingv1.VehicleManager)
	 */
	public void doTask(VehicleManager vm) throws InvalidRegistrationNumberException {
		...
		if (vm.checkingEmptySlots()) {
			...
			if (vm.searchVehicle(vehicleNumber)) {
				...
				System.out.println(vehicleNumber + " vehicle has successfully unparked");
			} else
				System.out.println(vehicleNumber + " vehicle has not present");
		} else
